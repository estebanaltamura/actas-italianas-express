...
export const useFormValidator = (e) => {  
  ...
  const isValidPhoneNumberEntry = (phoneNumber)=>{    
  ...
  const phoneNumberAlerts = (phoneNumber)=>{
    const digitRegExpVacia = /^$/;   
  ...
  const mailAlerts = (mail) => {
    ...
    if (dotAtStart.test(mail) || dotAtEnd.test(mail)) {
      setMailAlert(
        "La direccion de mail no puede empezar ni terminar con un punto (.)"
      );
      return;
    }
